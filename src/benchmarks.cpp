...
void Benchmark_RawVertexPulling::Init(const ig::IGLOContext& context, ig::CommandList& cmd, const Quad* src, uint32_t numQuads)
{
	...
	desc.primitiveTopology = ig::Primitive::TriangleList;
	desc.vertexLayout = {}; // No vertex layout is provided
	pipeline.Load(context, desc);
}
...
void Benchmark_ConfinedToGPU::Init(const ig::IGLOContext& context, ig::CommandList& cmd, const Quad* src, uint32_t numQuads)
{
	structuredBuffer.LoadAsStructuredBuffer(context, sizeof(Quad), numQuads, ig::BufferUsage::Default);
...
void Benchmark_ConfinedToGPU::OnRender(const ig::IGLOContext& context, ig::CommandList& cmd, const Quad* src, uint32_t numQuads)
{
	PushConstants pushConstants;
	pushConstants.screenSize = ig::Vector2((float)context.GetWidth(), (float)context.GetHeight());
	pushConstants.rawOrStructuredBufferIndex = structuredBuffer.GetDescriptor()->heapIndex;
